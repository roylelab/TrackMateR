...
#' @param ... pass additional parameters to modify the defaults (N, short,
#'   deltaT, mode, nPop, init, timeRes, breaks, radius)
#'
#' @return multiple pdf reports
#' @export
compareDatasets <- function(datadir = "Data", ...) {
  ...
  for (i in 1:length(condFolderNames)) {
    ...
    process_file <- function(j) {
      ...
      # perhaps this condition is too rigid?
      if (calibrationDF[5, 1] < 3 & calibrationDF[6, 1] < 10) {
        cat(paste0("Skipping ", fileName,
                   " as it has less than 3 tracks and
                   longest track has less than 10 frames\n"))
        return(NULL)
      }
      ...
      deltaT <- 1
      jdObj <- calculateJD(dataList = tmObj,
                           deltaT = l$deltaT,
                           nPop = l$nPop,
                           mode = l$mode,
                           init = l$init,
      ...
      fileNameBase <- sub(".xml", "", fileName)
      both <- makeSummaryReport(tmList = tmObj,
                                msdList = msdObj,
                                jumpList = jdObj,
                                tddf = tdDF,
                                fddf = fdDF,
                                titleStr = condFolderName,
                                subStr = fileNameBase,
                                auto = TRUE,
                                summary = FALSE,
                                msdplot = l$msdplot)
    ...
    for (res in results) {
      ...
      if (!is.null(res$calibrationDF))
        bigcalibration_list[[length(bigcalibration_list) + 1]] <-
          res$calibrationDF
      if (!is.null(res$msdDF))
        bigmsd_list[[length(bigmsd_list) + 1]] <- res$msdDF
      if (!is.null(res$alphaDF))
        bigalpha_list[[length(bigalpha_list) + 1]] <- res$alphaDF
      if (!is.null(res$deeDF))
        bigdee_list[[length(bigdee_list) + 1]] <- res$deeDF
      if (!is.null(res$jdDF))
        bigjd_list[[length(bigjd_list) + 1]] <- res$jdDF
      if (!is.null(res$jdParams))
        bigjdParams_list[[length(bigjdParams_list) + 1]] <- res$jdParams
      if (!is.null(res$tdDF))
        bigtd_list[[length(bigtd_list) + 1]] <- res$tdDF
      if (!is.null(res$fdDF))
        bigfd_list[[length(bigfd_list) + 1]] <- res$fdDF
      if (!is.null(res$df_report))
        bigreport_list[[length(bigreport_list) + 1]] <- res$df_report
    }

    bigtm <- if (length(bigtm_list) > 0)
      do.call(rbind, bigtm_list) else data.frame()
    bigmsd <- if (length(bigmsd_list) > 0)
      do.call(rbind, bigmsd_list) else data.frame()
    bigalpha <- if (length(bigalpha_list) > 0)
      do.call(rbind, bigalpha_list) else data.frame()
    bigdee <- if (length(bigdee_list) > 0)
      do.call(rbind, bigdee_list) else data.frame()
    bigjd <- if (length(bigjd_list) > 0)
      do.call(rbind, bigjd_list) else data.frame()
    bigtd <- if (length(bigtd_list) > 0)
      do.call(rbind, bigtd_list) else data.frame()
    bigfd <- if (length(bigfd_list) > 0)
      do.call(rbind, bigfd_list) else data.frame()
    bigreport <- if (length(bigreport_list) > 0)
      do.call(rbind, bigreport_list) else data.frame()
    ...
    if (is.null(summary_params) || length(summary_params) == 0) {
      summary_params <- if (length(valid_param_lists) > 0) {
        valid_param_lists[[1]]
        } else {
          list()
        }
    ...
    summaryObj <- makeSummaryReport(tmList = bigtmObj,
                                    msdList = bigmsdObj,
                                    jumpList = bigjdObj,
                                    tddf = bigtd,
                                    fddf = bigfd,
    ...
    # mega data frame of msd averages per dataset; alpha values, track density,
    # speed/duration/distance, intensity by trace/dataid/condition
    msdSummary <- summaryObj[[2]]
    msdSummary$condition <- condFolderName
    bigspeed <- bigtm %>%
      group_by(dataid, trace) %>%
      summarise(cumdist = max(cumulative_distance),
                cumtime = max(track_duration),
                intensity = max(mean_intensity))
  ...
  write.csv(megamsd,
            paste0(destinationDir, "/allMSDCurves.csv"), row.names = FALSE)
  write.csv(megareport,
            paste0(destinationDir, "/allComparison.csv"), row.names = FALSE)

  # for alpha values, estimator of D, track density, peed/duration/distance,
  # intensity by trace/dataid/condition we must combine into one
  # set the name of dee to estdee
  names(megadee)[names(megadee) == "dee"] <- "estdee"
  megatrace <- Reduce(mergeDataFramesForExport,
                      list(megaalpha, megadee, megatd, megaspeed, megafd))
  write.csv(megatrace, paste0(destinationDir, "/allTraceData.csv"),
            row.names = FALSE)

  # generate the comparison plots and save
  p <- makeComparison(df = megareport,
                      msddf = megamsd,
                      units = units_vec,
                      msdplot = l$msdplot)
